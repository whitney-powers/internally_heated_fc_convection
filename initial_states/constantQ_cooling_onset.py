...
problem.add_bc("left(T) = left(T0)")
problem.add_bc("right(Tz) = right(T0_z)")
...
# BCs
    ...
    #                (True, " left(w) = 0", "True"),
    #                (True, "right(w) = 0", "True"),
onset.add_bc(" right(T1_z) = 0")
onset.add_bc("left(T1) = 0")
...
nk = 20
nRa = 20
kpoints = np.linspace(1, 3, nk)
Rapoints = np.linspace(10, 1000, nRa)
...
if comm.rank == 0:
    ...
    pax.figure.savefig("constQ_cooling_2d_growth_rates_eps"+str(epsilon)+".png",dpi=300)
