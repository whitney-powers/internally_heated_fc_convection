...
The k=0 mode has a different diffusivity from the other modes
...
There are 6 control parameters:
    ...
    Ma      - The Mach number - derived from MLT
    nrho    - The height of the box
    aspect  - The aspect ratio (Lx = aspect * Lz)

Usage:
    polytrope_IH_k0_FC_2D.py [options] 
    polytrope_IH_k0_FC_2D.py <config> [options] 

Options:
    --Ra=<Ra>                  Flux Ra of convection [default: 1e5]
    --m=<m>                    The polytropic index of the radiative state [default: 1]
    --Pr=<Prandtl>             Prandtl number = nu/kappa [default: 1]
    --Ma=<Ma>                  Mach number [default: 1e-1]
...
#    kx_n0 = "(nx != 0) or  (ny != 0)"
    ...
    equations = ( (True, "True", "T1_z - dz(T1) = 0"),
                  ...
                  (True, kx_0, "T0*(dt(T1) + w*T0_z + (γ-1)*T0*Div_u - diff_L_0 ) = T0*(-UdotGrad(T1, T1_z) - (γ-1)*T1*Div_u + visc_heat + diff_R_0)"), #energy eqn
                  (True, kx_n0, "T0*(dt(T1) + w*T0_z + (γ-1)*T0*Div_u - diff_L_n0 ) = T0*(-UdotGrad(T1, T1_z) - (γ-1)*T1*Div_u + visc_heat + diff_R_n0)"), #energy eqn                        
...
def set_subs(problem):
    ...
    problem.substitutions['delta_T_ad'] = 'right(T_ad) - left(T_ad)'
    problem.substitutions['delta_T_rad'] = 'right(T_rad) - left(T_rad)'
    ...
    problem.substitutions['diff_L_0'] = '((κ0/(rho0*Cv))*Lap(T1, T1_z))'
    problem.substitutions['diff_R_0'] = '((1/(rho_full*Cv))*(Q + κ0*Lap(T, T_z)) - diff_L_0)'

    problem.substitutions['diff_L_n0'] = '((κ/(rho0*Cv))*Lap(T1, T1_z))'
    problem.substitutions['diff_R_n0'] = '((1/(rho_full*Cv))*(Q + κ*Lap(T, T_z)) - diff_L_n0)'
    ...
    problem.substitutions['grad_rad']  = '(flux/(R*κ0*g))'
    ...
    problem.substitutions['F_cond'] = '(-κ0*T_z)'
...
def run_cartesian_convection(args):
    ...
    data_dir += "_Ra{}_m{}_nrho{}_Pr{}_Ma{}_a{}_{}x{}".format(args['--Ra'], args['--m'], args['--nrho'], args['--Pr'], args['--Ma'], args['--aspect'], args['--nx'], args['--nz'])
    ...
    #MLT Mach Number
    Q_mag = Ma**3/delta_h
    F_conv = Q_mag*delta_h
        
    #Heating and diffusivities
    κ0 = -F_conv/(T_rad_z - T_ad_z)
    κ2μ = g * Lz**4 * F_conv *Cp / Ra #Pr = mu * cp / kappa, so  mu kappa = Pr * kappa**2 / cp
    κ = (κ2μ * Cp / Pr)**(1/3)
    μ = Pr * κ / Cp
    t_heat = (Lz**2/Q_mag)**(1/3)
    ...
    logger.info("   m = {:.3f}, Ra = {:.3e}, Pr = {:.2g}, Ma = {:.2g}, resolution = {}x{}, aspect = {}".format(rad_m, Ra, Pr, Ma, nx, nz, aspect))
    logger.info("   heating timescale: {:8.3e}, kappa_0 = {:.3e}, kappa = {:.3e}, mu = {:.3e}".format(t_heat, κ0, κ, μ))
    ...
    delta_T_rad = (T_rad_f.interpolate(z=Lz) - T_rad_f.interpolate(z=0)).evaluate()['g']
    delta_T_ad  = (T_ad_f.interpolate(z=Lz) - T_ad_f.interpolate(z=0)).evaluate()['g']
    if np.prod(delta_T_rad.shape)>0:
        delta_T_rad = delta_T_rad.min()
    if np.prod(delta_T_ad.shape)>0:
        delta_T_ad = delta_T_ad.min()
    ...
    Q.antidifferentiate('z', ('right', κ0), out=flux)
    ...
    problem.parameters['T_ad'] = T_ad_f
    problem.parameters['T_rad'] = T_rad_f
